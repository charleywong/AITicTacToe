...
int agent_second_move( int board_num, int prev_move )
{
  ...
  do {
    ...
    // check if center position of prev_move (board) is open
    if (board[prev_move][5] == EMPTY) {
      this_move = 5;
    }
    else {
      // pick a corner to play in (1,3,7,9)
      const int cornerChoices[4] = {1,3,7,9};
      int r = random()% 4; // make sure this space is empty
      this_move = cornerChoices[r];
      printf("corner choice\n");
      printf("%d\n", this_move);
    }
